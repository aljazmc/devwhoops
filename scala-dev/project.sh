...
if [ ! -f docker-compose.yml ]; then
  ...
  services:
    scala-compiler:
      image: sbtscala/scala-sbt:eclipse-temurin-alpine-22_36_1.10.0_3.4.2
...
runscala() {
  ...
  if [ ! -f HelloWorld.scala ]; then
    cat<<EOF > HelloWorld.scala
@main def hello() = println("Hello, World!")
EOF
  fi

  docker compose run scala-compiler sh -c "scalac HelloWorld.scala"
  docker compose run scala-compiler sh -c "scala hello"
